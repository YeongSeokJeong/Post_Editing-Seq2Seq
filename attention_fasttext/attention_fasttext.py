...
# for val_inp, val_targ in zip(val_input_tokens, val_output_toknes):
#     morphs_val_input.append(fasttext_output(val_inp))
#     morphs_val_output.append(fasttext_output(val_targ))

# for te_inp, te_targ in zip(test_input_tokens, test_output_tokens):
#     morphs_test_input.append(fasttext_output(te_inp))
#     morphs_test_output.append(fasttext_output(te_targ))
...
def fasttext_output(sentence):
    inp_sent = sentence
    if type(sentence[0]) is int:
        inp_sent = [input_vocab[i] for i in sentence]
    ft_output = [fasttext_model[word] for word in inp_sent]
    return ft_output
        ...
        if len(x.shape) == 1:
            ...
            for sentence in x:
                ...
                for word in sentence:
                    embed_sen.append(fasttext_model[input_vocab[word]])
                embed_output.append(embed_sen)
        return tf.convert_to_tensor(embed_output)
    ...
    def embedding(self, x):
        ...
        if len(x.shape) == 1:
            ...
            for sentence in x:
                embed_sen = []
                for word in sentence:
                    embed_sen.append(fasttext_model[output_vocab[word]])
                embed_output.append(embed_sen)

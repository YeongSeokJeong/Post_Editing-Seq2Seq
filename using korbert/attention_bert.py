...
print(val_input_token[0])
print(val_output_token[0])

print([ids_to_token_vocab[word] for word in val_input_token[0] if word not in [0,1,2,3]])
print([ids_to_token_vocab[word] for word in val_output_token[0] if word not in [0,1,2,3]])
...
def validation_loss(val_input = val_input_token,  val_output = val_output_token):
    ...
    for batch in range(int(len(val_input)/BATCH_SIZE)):
        ...
        for i in range(BATCH_SIZE):
            step_input = [ids_to_token_vocab[word] for word in test_input[i] if word not in [0, 1, 2, 3]]
            step_output = [ids_to_token_vocab[word] for word in test_output[i] if word not in [0, 1, 2, 3]]
            step_predict = [ids_to_token_vocab[word] for word in predict_morph[i] if word not in [0, 1, 2, 3]]
...
for epoch in tqdm(range(EPOCHS)):
    ...
    print("Input sentence Bleu Score : {:.4f}%".format(inp_wer))
...
with open("output_18Eopochs.txt", 'w') as f:
    for i in range(len(test_input_token)):
        inp, oup = translate(test_input_token[i])
        sen_inp = ''
        sen_cor = ''
        for word in inp:

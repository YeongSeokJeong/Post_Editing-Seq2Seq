...
class MultiHead_dot_product_Attention(tf.keras.layers.Layer):
    def __init__(self, batch_size, units, head_num):
        ...
        self.units = units
        self.split_head_units = int(units/head_num)
...
class Decoder(tf.keras.Model):
    def __init__(self,vocab_size, embedding_dim, dec_units, batch_sz):
        ...
        self.attention = MultiHead_dot_product_Attention(self.batch_sz, self.dec_units, 8)
...
def validation_loss(val_input = val_input_token,  val_output = val_output_token):
    total_loss = 0
    for batch in tqdm(range(int(len(val_input)/BATCH_SIZE))):
        loss = 0

...
class Decoder(tf.keras.Model):
    def __init__(self,vocab_size, embedding_dim, dec_units, batch_sz):
        ...
        self.start_dense = tf.keras.layers.Dense(embedding_dim)
    ...
    def make_start_token(self, enc_out):
        enc_out = tf.reshape(enc_out, (enc_out.shape[0], 1, -1))
        x = self.start_dense(enc_out)
        return x
...
def train_step(inp, targ, enc_hidden):
    ...
    with tf.GradientTape() as tape:
        ...
        dec_input = decoder.make_start_token(enc_output)
...
def validation_loss(val_input = val_input_token,  val_output = val_output_token):
    ...
    for batch in range(int(len(val_input)/BATCH_SIZE)):
        ...
        dec_hidden = enc_hidden
        dec_input = decoder.make_start_token(enc_out)
...
with open("start_token_change10.txt", 'w') as f:
